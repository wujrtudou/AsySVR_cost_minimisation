...
## In the simulation, New South Wales half-hourly electric load dataset is applied to validate our

## proposed asymmetric frameworks.

## import the NSW electric load data in Feburary, 2019

DataSet1<-read.csv("201902_NSW.csv")

## import the NSW electric load data in March, 2019

DataSet2<-read.csv("201903_NSW.csv")
...
Asy_SVR_Coef<-optim(as.array(Scale_LAV$coefficients), function(beta) 
  AsySVRStructure(beta, NScale_TrainSet[,1:50], NScale_TrainSet[,51], k=k, C=100, eps=0.001), 
  method="L-BFGS-B",lower=Scale_Low_Limt, upper = Scale_Upp_Limt)$par
...
write.csv(Output_Results,file="Model_Output_NSW1200.csv")
